...
class adaptivebeamfomer(beamformer):
    def __init__(self, mic: MicArray, frameLen=256, hop=None, nfft=None, c=343, r=0.032, fs=16000):

        beamformer.__init__(self, mic=mic, frame_len=frameLen, hop=hop, nfft=nfft, c=c, fs=fs)
        self.M = mic.M
        self.angle = np.array([197, 0]) / 180 * np.pi
        self.gamma = mic.gamma
    ...
    def process(self, x, angle, method=2, retH=False, retWNG=False, retDI=False):
        ...
        for t in range(0, frameNum):
            ...
            for k in range(0, self.half_bin):
                ...
                self.Ryy[k, :, :] = alpha_y * self.Ryy[k, :, :] + (1 - alpha_y) * np.dot(
                    Z[:, k, np.newaxis], Z[:, k, np.newaxis].conj().transpose()
                )
                Diagonal = 1e-6
                ...
                if self.update_noise_psd_flag:
                    self.Rvv[k, :, :] = alpha_v * self.Rvv[k, :, :] + (1 - alpha_v) * np.dot(
                        Z[:, k, np.newaxis], Z[:, k, np.newaxis].conj().transpose()
                    )
                    self.update_noise_psd_flag = 0
    ...
    def AdaptiveMVDR2(self, x, angle):
        ...
        for t in range(0, frameNum):
            ...
            if t < 200:
                for k in range(0, self.half_bin):
                    Rvv[k, :, :] = alpha * Rvv[k, :, :] + (1 - alpha) * np.dot(
                        Z[:, k, np.newaxis], Z[:, k, np.newaxis].conj().transpose()
                    )
                    a = np.mat(np.exp(-1j * self.omega[k] * tao)).T  # propagation vector
                    H[:, k] = self.getMVDRweight(a, Rvv[k, :, :])
    ...
    def AdaptiveMVDR(self, x, angle, retH=True, retWNG=True, retDI=True):
        ...
        for k in range(0, self.half_bin):
            for t in range(0, 200):
                Rvv[k, :, :] = alpha * Rvv[k, :, :] + (1 - alpha) * np.dot(
                    Zxx[:, k, t, np.newaxis], Zxx[:, k, t, np.newaxis].conj().transpose()
                ) / (self.win_scale**2)
        ...
        return {'out': xrec, 'WNG': WNG, 'DI': DI, 'beampattern': beampattern}
